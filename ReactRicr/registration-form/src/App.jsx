...
const App = () => {
  ...
  const validatate = () => {
    let error={};
    if(!SubmitRegistrationForm.firstname.length <3){
      error.firstname="First Name must be at least 3 characters long";
    }else{
      if(!/^[A-Za-z]+$/.test(SubmitRegistrationForm.firstname)){
        error.firstname="First Name must contain only letters";
      }
      {
    if(!SubmitRegistrationForm.lastname.length <3){
      error.lastname="Last Name must be at least 3 characters long";
    }else{
      if(!/^[A-Za-z]+$/.test(SubmitRegistrationForm.lastname)){
        error.lastname="Last Name must contain only letters";
      }
    }
      }
    }
    if(!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(SubmitRegistrationForm.email)){
      error.email="Invalid Email Format";
    }
    if(!/^[6-9]\d{9}$/.test(SubmitRegistrationForm.number)){
      error.number="Invalid Phone Number";
    }
    if(SubmitRegistrationForm.State==="--Select--" || SubmitRegistrationForm.State===""){
      error.State="Please select a State";
    }
    if(SubmitRegistrationForm.District==="--Select--" || SubmitRegistrationForm.District===""){
        ...
        <div className="container border w-200 h-160 border-blue-300 flex mx-auto mt-15 rounded-2xl shadow-lg">
          <form onSubmit={SubmitData} onReset={ClearForm}>
            ...
            <div className="flex gap-5 justify-center mt-3 ms-5">
              <div>
                <label htmlFor="firstname" className="text-2xl p-3 mt-1">
                  First Name
                </label>
               <div>
                 <input
                  ...
                  className="border h-fit p-1 mt-2 w-90 rounded-[7px] border-blue-400 shadow-lg"
                />
              ...
              <div>
                <label htmlFor="lastname" className="text-2xl p-3 mt-1">
                  Last Name
                </label>
                <input
                  type="text"
                  ...
                  className="border p-1 h-fit mt-2 w-90 rounded-[7px] border-blue-400 shadow-lg"
                />
              </div>
            </div>
            <div>
              <label htmlFor="email" className="text-xl p-3 mt-1 ms-5">
                Enter Email
              </label>
              <input
                type="email"
                ...
                className="border p-1 mt-8 w-160 h-fit rounded-[7px] border-blue-400 shadow-lg"
              />
            </div>
            <div>
              <label htmlFor="number" className="text-xl p-3 mt-1">
                Your Number
              </label>
              <input
                type="number"
            ...
            <div>
              <label htmlFor="dob" className="text-xl p-3 mt-1">
                Date-Of-Birth
              </label>
              <input
                type="date"
            ...
            <div className="flex gap-2 justify-center mt-3">
              <div className="me-25">
                <label htmlFor="State" className="text-xl me-4">
                  State
                </label>
                <select
                  name="State"
              ...
              <div>
                <label htmlFor="District" className="text-xl me-5">
                  District
                </label>
                <select
                  name="District"
            ...
            <div className="flex gap-5 mt-8">
              <label htmlFor="gender" className="text-2xl ms-8 ">
                Gender
              </label>
              <input
                type="radio"
            ...
            <div className="flex mt-1 gap-8">
              <div className="text-2xl mt-1 ms-8">
            ...
            <div className="flex gap-5 justify-center mt-5 mb-5">
              <button
                type="submit"
                className="border p-3 rounded-2xl text-blue-300 border-blue-500 hover:border-amber-200 shadow-lg"
              >
                Submit Data
              </button>
              <button
